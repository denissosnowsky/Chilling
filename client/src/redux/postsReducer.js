...
const postsReducer = (state = initialState, action) => {
    switch(action.type){
        ...
        case ADD_TEMP_LIKE:
            return{
                ...state,
                posts: state.posts.map(post=>{ if(post._id==action.id){return {...post, liked: [...post.liked, action.myId]}}else{return {...post}}})
            }
        case ADD_TEMP_DISLIKE:
            return{
                ...state,
                posts: state.posts.map(post=>{ if(post._id==action.id){return {...post, liked: post.liked.filter(id=>id!=action.myId)}}else{return {...post}}})
            }
...
export const getPostsThunk = (id, portion, currentPage, id_post) => async (dispatch) => {
...
export const addPostThunk = (img, text, to, portion, currentPage) => async (dispatch) => {
...
export const removePostThunk = (id, to, portion, currentPage) => async (dispatch) => {
...
export const likePostThunk = (id, me, to, portion, currentPage) => async (dispatch) => {
...
export const dislikePostThunk = (id, me, to, portion, currentPage) => async (dispatch) => {
