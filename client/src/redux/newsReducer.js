...
const newsReducer = (state=initialState, action) => {
    switch(action.type){
        ...
        case ADD_TEMP_LIKE_NEW:
            return{
                ...state,
                news: state.news.map(post=>{ if(post._id==action.id){return {...post, liked: [...post.liked, action.myId]}}else{return {...post}}})
            }
        case ADD_TEMP_DISLIKE_NEW:
            return{
                ...state,
                news: state.news.map(post=>{ if(post._id==action.id){return {...post, liked: post.liked.filter(id=>id!=action.myId)}}else{return {...post}}})
            }
...
export const getNewsThunk = (portion, currentPage, dial_id) => async (dispatch) => {
...
export const removeNewThunk = (id, to, portion, currentPage) => async (dispatch) => {
...
export const likeNewThunk = (id, me, to, portion, currentPage) => async (dispatch) => {
...
export const dislikeNewThunk = (id, me, to, portion, currentPage) => async (dispatch) => {
