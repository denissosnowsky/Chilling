...
export const getMessagesMenuThunk =
  (portion, currentPage) => async (dispatch) => {
...
export const getMessagesMenuThunkUpdate =
  (portion, currentPage) => async (dispatch) => {
    const res = await messageAPI.getMessagesMenu(portion, currentPage);
    if (res.resultCode === 0) {
      console.log(res.message);
...
export const getMessagesThunk =
  (id, portion, currentPage) => async (dispatch) => {
...
export const getMessagesThunkUpdate =
  (id, portion, currentPage) => async (dispatch) => {
    const res = await messageAPI.getMessages(id, portion, currentPage);
    if (res.resultCode === 0) {
      console.log(res.message);
...
export const addMessageThunk =
  (img, text, to, id, portion, currentPage, myId) => async (dispatch) => {
    const res = await messageAPI.addMesagge(img, text, to);
    if (res.resultCode === 0) {
      console.log(res.message);
    } else {
      dispatch(getMessagesThunkUpdate(id, portion, currentPage));
      dispatch(sendMessageThunk({ myId, id, portion, currentPage }));
    }
  };
...
const newMessageHandlerCreator = (dispatch) => {
  return (messages) => {
    dispatch(getMessges(messages));
  };
};

const newCountHandlerCreator = (dispatch) => {
  return (count) => {
    dispatch(countMessages(count));
  };
};

export const startMessagesListening = () => async (dispatch) => {
  chatAPI.subscribe("messages", newMessageHandlerCreator(dispatch));
  chatAPI.subscribe("count", newCountHandlerCreator(dispatch));
  chatAPI.listen();
};
...
export const joinToDialogThunk = (id) => async (dispatch) => {
  chatAPI.joinToDialog(id);
};

export const leaveTheDialogThunk = (id) => async (dispatch) => {
  chatAPI.leaveTheDialog(id);
};
