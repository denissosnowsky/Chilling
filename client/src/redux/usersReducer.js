...
export const getUsersThunk = (pageNumber, pageSize, searchedPhrase) => async (dispatch) =>{
...
export const getUsersThunkUpdate = (pageNumber, pageSize, searchedPhrase) => async (dispatch) =>{
    const res = await usersAPI.getUsers(pageNumber, pageSize, searchedPhrase);
    if(res.resultCode === 0){
        console.log(res.message);
    } else {
