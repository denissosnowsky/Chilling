    ...
    <div className={s.user}>
      <div className={s.user__wallpaper}>
        {props.isFetchingBImage ? (
          ...
          <img
            ...
            src={props.user.lImg ? url + props.user.lImg : wallpaperPhoto}
            alt="user"
          />
        )}
        {props.paramId == props.me && (
          <>
            <input
              className={s.user__inputBig}
              accept="image/*"
              type={"file"}
              name="lImg"
              id="lImg"
              onChange={props.selectWallpaper}
            />
            <label className={s.user__labelBig} htmlFor="lImg">
              Змінити фон
            </label>
          </>
        )}
      ...
      <div className={s.user__userPhoto}>
        {props.isFetchingSImage ? (
          ...
          <img
            src={props.user.sImg ? url + props.user.sImg : logoPhoto}
            alt="user"
          />
        )}
        {props.paramId == props.me && (
          <>
            <input
              className={s.user__inputSmall}
              accept="image/*"
              type={"file"}
              name="sImg"
              id="sImg"
              onChange={props.selectPhoto}
            />
            <label className={s.user__labelSmall} htmlFor="sImg">
              Змінити фото
            </label>
          </>
        )}
      ...
      <div className={s.user__userDetails}>
        <div className={cs([s.user__name], "border", "border-1")}>
          ...
          {props.paramId != props.me && (
            <button
              onClick={() => {
                props.createDialog(props.user._id);
              }}
              type="button"
