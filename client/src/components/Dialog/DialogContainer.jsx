...
import { addMessageThunk, fetchingLoadMessage, getMessagesThunk, getMessagesThunkUpdate, setDialogId, setIsTempMessage, setIsTempPic, setTempMessage, setTempPict, shouldUpadateFunc } from '../../redux/messageReducer';
...
const DialogContainer = ({getMessagesThunk, messages, myId, menuMessage, speakers, addMessageThunk, setTempPict, tempPic, setIsTempPic, isTempPic, setTempMessage, isTempMessage, setIsTempMessage, messageCount, portionMessages, shouldUpadate, shouldUpadateFunc, fetchingLoadMessage, isFetchingLoadMessage, isFetchingMessages, getMessagesThunkUpdate, foundId, setDialogId, ...props}) => {
    ...
    useEffect(()=>{
        if(shouldUpadate){
            if(wrapper.current.scrollHeight-wrapper.current.clientHeight-wrapper.current.scrollTop<=wrapper.current.clientHeight+100 || wrapper.current.scrollTop==0){
                wrapper.current.scrollTop = wrapper.current.scrollHeight;
            }
        }else{
            shouldUpadateFunc(true);
            wrapper.current.scrollTop = wrapper.current.scrollHeight-prevScrollHeight.current-wrapper.current.clientHeight+200;
        }
    ...
    useEffect(() => {
        getMessagesThunk(id, portionMessages, currentPage);
            wrapper.current.scrollTop = wrapper.current.scrollHeight;
        // 5s reg requests for messages updating
    ...
    const submitForm = (event) => {
        ...
        addMessageThunk(img, text, to, id, portionMessages, currentPage);
        setText('');
        setTempPict('');
        setIsTempPic(false);
        setTempMessage({from: {_id: myId}, text, img: tempImg, date: Date.now()});
...
export default compose(withRouter, connect(mapStateToProps, {getMessagesThunk, addMessageThunk, setTempPict, setIsTempPic, setTempMessage, setIsTempMessage, shouldUpadateFunc, fetchingLoadMessage, getMessagesThunkUpdate, setDialogId}))(DialogContainer);
