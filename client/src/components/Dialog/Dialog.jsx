      ...
      <div className={cs([s.dialog__wrapper], "border", "border-1")}>
        <div ref={wrapper} className={cs([s.dialog__messeges], "scrollbar")}>
          <div className={cs([s.dialog__filled])}>
            {isFetchingMessages ? (
              <div className={s.dialog__preloader}>
                <Spinner />
              </div>
            ) : messages.length > 0 ? (
              messages.map((item, i) => {
              ...
              <div className={s.dialog__noMessages}>
                У Вас поки немає повідолмень
              </div>
            )}
            {currentPage < pagesNumber && (
              ...
              >
                {isFetchingLoadMessage ? (
                  <img
                    src={spinner}
                    className={s.dialog__spinnerBtn}
                    alt="spinner"
                  />
                ) : (
                  "Загрузити ще"
        ...
        <div className={cs([s.dialog__photo], "border", "border-1")}>
          <img alt="speaker" src={speakerImg ? speakerImg : userPhoto} />
        </div>
        <form ref={formRef} className={cs([s.dialog__panel])} action="#">
          <textarea
            ...
            autoFocus={true}
            className={cs([s.dialog__textarea], "border", "border-1")}
