  ...
  return (
    <div>
      <div className={cs([s.posts__post], "border", "border-1")}>
        <div
          onDoubleClick={() => {
            likeToggle(post._id, myId, post.to._id, portion, currentPage);
          }}
          className={s.posts__like}
        >
        ...
        {post.from._id == myId || post.to._id == myId ? (
          isFetchingPostDelete.some((id) => id == post._id) ? (
            <img
              src={spinner}
              alt="spinner"
              className={cs([s.posts__delete, s.posts__spinnerImg])}
            />
          ) : (
            <div
              onClick={() => {
                removePost(post._id, post.to._id, portion, currentPage);
              }}
              className={s.posts__delete}
            >
        ...
        <NavLink
          to={`/profile/${post.from.order}`}
          className={s.posts__userLogo}
        >
          <img
            src={post.from.sImg ? url + post.from.sImg : logoPhoto}
            alt="logo"
          />
        </NavLink>
        {pageFrom === "news" ? (
          <NavLink
            to={`/profile/${post.to.order}`}
            className={s.posts__receiverLogo}
          >
            <img
              src={post.to.sImg ? url + post.to.sImg : logoPhoto}
              alt="logo"
            />
          </NavLink>
        ) : null}
        ...
        <div className={s.posts__text}>{post.text}</div>
      </div>
    </div>
  );
};
