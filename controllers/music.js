...
export const getAllMusic = async (req, res) => {
    try{
        ...
        if(search){
            music = music.filter(item=> item.name.toLowerCase().indexOf(search.toLowerCase())!=-1 || item.author.toLowerCase().indexOf(search.toLowerCase())!=-1).reverse();
        } else {
            music = music.reverse();
        }
...
export const addMusic = async (req, res) => {
    try{
        const music = await User.findOneAndUpdate({_id: req.user.userId}, {$push: {music: req.body.id,},});
...
export const deleteMusic = async (req, res) => {
    try{
        const music = await User.findOneAndUpdate({_id: req.user.userId}, {$pull: {music: req.body.id,},});
