...
export const friends = async (req, res) => {
  try {
    ...
    const friendsData = await User.findOne({ _id: req.user.userId })
      .populate("friends")
      .select("friends");

    if (searchedPhrase) {
      filteredArray = friendsData.friends
        .filter(
          (item) =>
            item.name.toLowerCase().indexOf(searchedPhrase.toLowerCase()) !=
              -1 ||
            item.surname.toLowerCase().indexOf(searchedPhrase.toLowerCase()) !=
              -1
        )
        .reverse();
    } else {
    ...
    const friendsArray = filteredArray.filter(
      (friend, i) => i >= previousUsers && i < nextUsers
    );
