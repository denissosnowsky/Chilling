...
export const getMessageMenu = async (req, res) => {
  try {
    ...
    let dialogs = await Dialog.find({
      $or: [{ speakerOne: from }, { speakerTwo: from }],
    })
      .populate("messages")
      .populate("speakerOne")
      .populate("speakerTwo");
    const count = await Dialog.find({
      $or: [{ speakerOne: from }, { speakerTwo: from }],
    }).count();
...
export const getMessages = async (req, res) => {
  try {
    ...
    const result = await getMessagesService(me, dial_id, portion, currentPage);

    const obj = {
      allMessages: result.data.allMessages,
      foundId: result.data.foundId,
      count: result.data.count,
      speakers: result.data.speakers,
    };
    res.status(result.code).json(obj);
  } catch (e) {
    res
      .status(500)
      .json({ message: "Щось пішло не так, спробуйте ще раз", resultCode: 0 });
...
export const addMessage = async (req, res) => {
  try {
    ...
    if (img || text) {
      ...
      const dialogOne = await Dialog.findOne({
        speakerOne: from,
        speakerTwo: to,
      });
      const dialogTwo = await Dialog.findOne({
        speakerOne: to,
        speakerTwo: from,
      });
      ...
      if (!(dialogOne || dialogTwo)) {
        ...
        const msg = await Dialog.findOneAndUpdate(
          { _id: dialogOne._id },
      ...
      res
        .status(201)
        .json({ message: "Повідомлення відправлено", resultCode: 1 });
    } else {
      res.status(500).json({
        message: "Не можна відправляти пусте повідолмення",
        resultCode: 0,
      });
    }
  } catch (e) {
