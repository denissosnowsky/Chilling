...
export const postAdd = async (req, res) => {
    try{
        ...
        if(img || text){
            ...
            await post.save();
            res.status(201).json({ message: 'Пост успішно створений', resultCode: 1});
        } else{
            res.status(500).json({message: 'Не можна створювати пустий пост', resultCode: 0});    
        }
    }catch(e){
...
export const getPosts = async (req, res) => {
    try{
        ...
        const posts = await Post.find({to: id}).populate('from').populate('to').sort({date: -1}).limit(portion*currentPage);
...
export const likePost = async (req, res) => {
    try{
        ...
        const like = await Post.findOneAndUpdate({_id: req.body.id}, {$push: {liked: req.body.me,},});
...
export const dislikePost = async (req, res) => {
    try{
        ...
        const dislike = await Post.findOneAndUpdate({_id: req.body.id}, {$pull: {liked: req.body.me,},});
